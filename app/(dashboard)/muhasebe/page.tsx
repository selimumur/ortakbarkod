...
import {
   ...
   Activity, CreditCard, Building2, PieChart,
...
import { getAccountingDashboardDataAction, type AccountingStats } from '@/app/actions/accountingActions';
import { toast } from 'sonner';

export default function AccountingDashboard() {
   const { userId, orgId } = useAuth(); // Client side check only
   ...
   async function fetchDashboardData() {
      ...
      try {
         ...
         setRecentInvoices([]); // TODO: Invoice refactor later
         setChartData(data.chartData);
      } catch (error: any) {
         console.error("Dashboard verisi çekilirken hata:", error);
         toast.error("Finansal veriler yüklenemedi: " + error.message);
      } finally {
         setLoading(false);
      }
   }
            ...
            <div className="lg:col-span-2 bg-[#111827] border border-gray-800 rounded-3xl p-8 shadow-2xl">
               <div className="flex items-center justify-between mb-8">
                  ...
                  <div className="flex gap-4 text-sm font-bold">
                     <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-blue-500"></div> Gelir</div>
                     <div className="flex items-center gap-2"><div className="w-3 h-3 rounded-full bg-red-500"></div> Gider</div>
                  </div>
               </div>
               <div className="h-[350px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                     <AreaChart data={chartData}>
                        ...
                        />
                        <Area type="monotone" dataKey="gelir" stroke="#3B82F6" strokeWidth={4} fillOpacity={1} fill="url(#colorSatis)" name="Toplam Gelir" />
                        <Area type="monotone" dataKey="gider" stroke="#EF4444" strokeWidth={4} fillOpacity={1} fill="url(#colorAlis)" name="Toplam Gider" />
                     </AreaChart>
                  </ResponsiveContainer>
               </div>
            </div>
                        ...
                        recentInvoices.map(inv => (
                           <tr key={inv.id} className="hover:bg-gray-800/30 transition group">
                              ...
                              <td className="p-4 text-center">
                                 <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${inv.invoice_type === 'sales'
