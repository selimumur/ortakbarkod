...
import { ArrowLeft, Landmark, Copy, Filter, Loader2, ArrowUpCircle, ArrowDownCircle, RefreshCw } from 'lucide-react';
import { toast } from 'sonner';
import { getAccountDetailsAction } from '@/app/actions/accountingActions';

export default function BankDetailPage(props: { params: Promise<{ id: string }> }) {
    ...
    async function fetchData() {
        ...
        try {
            ...
            setBank(data.account);
            setTransactions(data.transactions);
        } catch (error: any) {
            console.error("Banka detayları hatası:", error);
            toast.error("Banka bilgileri alınamadı: " + error.message);
    ...
    if (!bank) return <div className="min-h-screen bg-[#0f1115] flex items-center justify-center text-white">Hesap bulunamadı.</div>;
            ...
            <div className="mb-8">
                ...
                <div className="flex flex-col md:flex-row justify-between md:items-end gap-6 bg-[#1a1d24] p-6 rounded-2xl border border-gray-800 shadow-xl">
                    <div className="flex items-center gap-6">
                        <div className="w-20 h-20 bg-blue-500/10 rounded-2xl flex items-center justify-center text-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.1)]">
                            <Landmark size={40} />
                        </div>
                        <div>
                            <h1 className="text-3xl font-black text-white">{bank.name}</h1>
                            <p className="text-xl text-gray-400 font-medium">{bank.bank_name || 'Banka Hesabı'}</p>
                            {bank.iban && (
                                <div className="flex items-center gap-2 mt-2 bg-[#0B1120] px-3 py-1.5 rounded-lg border border-gray-700 w-fit cursor-pointer hover:border-gray-500 transition group" onClick={() => copyToClipboard(bank.iban)}>
                                    <span className="font-mono text-gray-300 tracking-wider group-hover:text-white">{bank.iban}</span>
                    ...
                    <div className="text-right">
                        ...
                        <p className={`text-4xl font-black ${Number(bank.balance) >= 0 ? 'text-white' : 'text-red-500'}`}>
                            {Number(bank.balance).toLocaleString('tr-TR', { minimumFractionDigits: 2 })} <span className="text-lg text-gray-500">{bank.currency}</span>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-[#1a1d24] border border-gray-800 rounded-2xl overflow-hidden shadow-xl">
                <div className="p-4 border-b border-gray-800 flex items-center justify-between">
                    <h3 className="font-bold text-white flex items-center gap-2">
                        <Filter size={18} className="text-blue-500" /> Hesap Hareketleri
                    </h3>
                </div>
