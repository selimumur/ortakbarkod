...
import { Shield, Lock, Link as LinkIcon, Plus, X, Store, CheckCircle, RefreshCw, ShoppingBag, Flower2, Globe, Trash2, Edit2 } from 'lucide-react';
...
export default function IntegrationsPage() {
   ...
   const openModal = (platformName: string, accountToEdit: any = null) => {
      setSelectedPlatform(platformName);
      if (accountToEdit) {
         setEditingId(accountToEdit.id);
         setFormData({
            storeName: accountToEdit.store_name,
            supplierId: accountToEdit.supplier_id,
            apiKey: accountToEdit.api_key,
            apiSecret: accountToEdit.api_secret
         });
      } else {
         setEditingId(null);
   ...
   const handleSave = async () => {
      ...
      try {
         if (editingId) {
            // Update
            await updateMarketplaceAction(editingId, {
   ...
   const handleDelete = async (id: string) => {
      ...
      try {
         await deleteMarketplaceAction(id);
            ...
            {accounts.map((acc) => (
               <div key={acc.id} className="bg-[#111827] border border-gray-800 rounded-xl p-4 flex flex-col md:flex-row items-center justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                     <button
                        onClick={() => openModal(acc.platform, acc)}
                        className="p-2 text-gray-400 hover:text-blue-400 transition bg-gray-800/50 rounded-lg"
                        title="Düzenle"
                     >
                        <Edit2 size={16} />
                     </button>
                     <button
                        ...
                        className="p-2 text-gray-400 hover:text-red-500 transition bg-gray-800/50 rounded-lg"
                        title="Sil"
                     >
                        <Trash2 size={16} />
                     </button>
            ...
            <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in zoom-in-95 duration-200">
               <div className="bg-[#111827] border border-gray-700 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
                  <div className="bg-gray-900/50 p-6 border-b border-gray-800 flex justify-between items-center">
                     <h3 className="text-white font-bold text-lg">{selectedPlatform} {editingId ? 'Güncelle' : 'Bağla'}</h3>
                  ...
                  <div className="p-6 space-y-4">
                     ...
                     <div>
                        <label className="text-xs text-gray-400 block mb-1.5">{selectedPlatform === 'WooCommerce' ? 'Web Site Adresi (URL) - (https://siteadi.com)' : 'Satıcı ID (Supplier ID)'}</label>
                  ...
                  <div className="p-5 border-t border-gray-800 flex justify-end gap-3 bg-gray-900/30">
                     ...
                     <button onClick={handleSave} disabled={loading} className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-bold rounded-lg transition flex items-center gap-2">
                        ...
                        {loading ? 'İşleniyor...' : (editingId ? 'Güncelle' : 'Bağlantıyı Kur')}
                     </button>
                  </div>
               </div>
            </div>
