...
import { Package, ShoppingCart, Barcode, RefreshCw, Search, X, MapPin, Store, ChevronDown, Loader2, Flower2, ShoppingBag, Filter, ExternalLink, ChevronLeft, ChevronRight, Calendar } from 'lucide-react';
import { toast } from 'sonner';
// HATALI SATIR SİLİNDİ: import { supabase } from '../utils/supabase/client';
import { createBrowserClient } from '@supabase/ssr'; // DOĞRUSU BU
...
export default function OrderManagement() {
  ...
  // --- SUPABASE CLIENT OLUŞTURMA (GARANTİ YÖNTEM) ---
  ...
  useEffect(() => {
    async function fetchAccounts() {
      const { data } = await supabase.from('marketplace_accounts').select('*').order('created_at');
  ...
  }, [selectedStoreId, activeTab, searchTerm, page, supabase]); // supabase dependency eklendi
