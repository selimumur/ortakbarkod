...
export async function POST(request: Request) {
    try {
        const { productIds, accountId, stock } = await request.json();
        ...
        const items = products.map((p: any) => {
            ...
            return {
                barcode: p.barcode,
                quantity: stock // Set fixed stock for all selected
            };
        }).filter(Boolean);
        ...
        if (response.ok) {
            return NextResponse.json({ success: true, message: "Stok güncellemesi kuyruğa alındı.", batchRequestId: result.batchRequestId });
        } else {
